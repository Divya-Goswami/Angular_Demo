...
abstract class REST_Controller extends CI_Controller {
	...
	public function _remap($object_called, $arguments) {
		...
		$log_method = ! (isset($this->methods[$controller_method]['log']) AND FALSE == $this->methods[$controller_method]['log']);

		// Use keys for this method?
		$use_key = ! (isset($this->methods[$controller_method]['key']) AND FALSE == $this->methods[$controller_method]['key']);

		// Get that useless shitty key out of here
		if (config_item('rest_enable_keys') AND $use_key AND FALSE === $this->_allow) {
	...
	public function response($data = [], $http_code = null) {
		...
		// Otherwise (if no data but 200 provided) or some data, carry on camping!
			...
			if ($CFG->item('compress_output') === TRUE && FALSE == $this->_zlib_oc) {
	...
	*/
			...
			foreach (array_keys($this->_supported_formats) as $format) {
				...
				if (strpos($this->input->server('HTTP_ACCEPT'), $format) !== FALSE) {
					...
					// HTML or XML have shown up as a match
						...
						if ('html' == $format AND strpos($this->input->server('HTTP_ACCEPT'), 'xml') === FALSE) {
							return $format;
						}

						// If it is truly XML, it wont want any HTML
						elseif ('xml' == $format AND strpos($this->input->server('HTTP_ACCEPT'), 'html') === FALSE) {
							return $format;
						}
					}
				}
	...
	protected function _log_request($authorized = FALSE) {
		return $this->rest->db->insert(config_item('rest_logs_table'), [
	...
	protected function _auth_override_check() {
		...
		if ('none' == $this->overrides_array[$this->router->class][$this->router->method]) {
			return true;
		}

		// Basic auth override found, prepare basic
		if ('basic' == $this->overrides_array[$this->router->class][$this->router->method]) {
			$this->_prepare_basic_auth();
		...
		if ('digest' == $this->overrides_array[$this->router->class][$this->router->method]) {
			$this->_prepare_digest_auth();
		...
		if ('whitelist' == $this->overrides_array[$this->router->class][$this->router->method]) {
			$this->_check_whitelist_auth();
	...
	protected function _check_login($username = '', $password = NULL) {
		...
		if (NULL !== $password AND $valid_logins[$username] != $password) {
			return FALSE;
		}
